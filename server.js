    ...
    await fetch('https://api.notion.com/v1/pages', {
      ...
      body: JSON.stringify({
        ...
        properties: {
          ...
          'Дата отзыва': { date: { start: new Date().toISOString().split('T')[0] } },
...
app.post('/api/admin/checklist-complete', async (req, res) => {
  if (!checkAdminPin(req, res)) return;
  const { type, items } = req.body; // type: 'открытие' | 'закрытие'

  const time = new Date().toLocaleString('ru-RU', { hour: '2-digit', minute: '2-digit', day: 'numeric', month: 'long' });
  const itemsList = (items || []).map(i => `✓ ${i}`).join('\n');

  await sendTelegramMessage(ADMIN_CHAT_ID, `📋 Чек-лист «${type}» выполнен — ${time}\n\n${itemsList}`);
  res.json({ ok: true });
});
